...
- When F-Droid developer-binary verification uses Android SDK Build-Tools 35.0.0 `apksigner`, sign the exact reproducible unsigned APK with `--alignment-preserved`. Default Build-Tools 35.0.0 signing can rewrite ZIP alignment extra fields and break byte-for-byte signature-copy reconstruction even when APK entry contents are unchanged.
